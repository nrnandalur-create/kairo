...
function computeRisk({ signals, bb, price, priceChange5d, hi52, lo52, net, bullishScore, bearishScore }) {
  ...
  // Overextension beyond a band raises risk regardless of direction.
  const bbSig = signals.find(s => s.key === 'bollinger')
  if (bbSig?.bbDescriptor?.zone === 'above_upper' || bbSig?.bbDescriptor?.zone === 'below_lower') {
    risk += 14; reasons.push('price is trading beyond a Bollinger Band')
  }
  // Sharp recent moves.
  const p = typeof priceChange5d === 'number' ? priceChange5d : parseFloat(priceChange5d)
...
function strongest(signals, direction) {
  return signals
    .filter(s => s.direction === direction)
    .sort((a, b) => b.strength - a.strength)[0] || null
}

function buildNarrative({ verdictLabel, direction, signals, risk, support, resistance }) {
  const bull = strongest(signals, 'bullish')
  const bear = strongest(signals, 'bearish')

  // WHY — strongest bullish + strongest bearish evidence + why they net out.
  const parts = []
  if (bull) parts.push(bull.explanation)
  if (bear) parts.push(bear.explanation)
  if (direction === 'neutral') {
    parts.push('Because the bullish and bearish evidence are closely matched, Kairo favors holding rather than aggressively adding or exiting.')
  } else if (direction === 'bullish') {
    parts.push(`On balance the bullish evidence outweighs the bearish, so Kairo leans ${verdictLabel.toLowerCase()}.`)
  } else {
    parts.push(`On balance the bearish evidence outweighs the bullish, so Kairo leans ${verdictLabel.toLowerCase()}.`)
  }
  ...
  // WHAT WOULD CHANGE THE THESIS — anchored to real levels when available.
  const nearSupport = Array.isArray(support) && support.length ? support[0] : null
  const nearRes = Array.isArray(resistance) && resistance.length ? resistance[0] : null
  const moreBullishIf = nearRes != null
    ? `Price reclaims and holds above ${fmt(nearRes)} while momentum strengthens.`
    : 'Price holds above its key moving averages while momentum strengthens.'
  const moreBearishIf = nearSupport != null
    ? `Price loses ${fmt(nearSupport)} while momentum and trend deteriorate.`
    : 'Price breaks below its recent support while momentum and trend deteriorate.'

  return {
    why,
    primaryRisk,
    whatWouldChange: { moreBullishIf, moreBearishIf },
  }
}
...
function validate(result) {
  ...
  const texts = [result.narrative.why, result.narrative.primaryRisk,
    result.narrative.whatWouldChange.moreBullishIf, result.narrative.whatWouldChange.moreBearishIf,
    ...(result.nextSteps || [])]
...
export function buildDecision(input = {}) {
  ...
  const narrative = buildNarrative({ verdictLabel: band.label, direction: band.direction, confidence: conf.value, signals, risk, support, resistance, price })
