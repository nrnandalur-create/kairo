...
export default async function handler(req, res) {
  // Authorization is enforced centrally in api/cron.js (Bearer CRON_SECRET).
  ...
  // After the brief round, chain-invoke the smart-signals evaluator so all
  // opted-in users get their proactive alert scan on the same daily fire.
  // Runs BEHIND the brief loop so brief email delivery isn't blocked on
  // signal evaluation latency. Errors here don't touch the brief result.
  let signalsResult = null
  try {
    const smartSignalsMod = await import('./smartSignals.js')
    // Fabricate a minimal req/res that satisfies the signal handler's shape.
    // This is a trusted in-process call (we're already past the cron auth gate
    // in api/cron.js), so no auth header is needed; we just mock res.json.
    const capturedRes = { status(s) { this._status = s; return this }, json(o) { this._body = o; return this }, _status: 200, _body: null }
    await smartSignalsMod.default({ headers: {}, query: {} }, capturedRes)
    signalsResult = capturedRes._body
  } catch (err) {
    signalsResult = { ok: false, error: err?.message ?? 'chain-invoke failed' }
  }

  res.json({ ok: true, sent, skipped, signals: signalsResult })
}
